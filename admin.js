/**
 * admin.js - Painel Administrativo (v3.0) - SEM VERIFICAÇÃO DE LOGIN
 * Dashboard, Estatísticas, Gestão de Pedidos
 */

class AdminPanel {
  constructor() {
    // Removido checkAccess() - acesso livre via Ctrl+Shift+A
    this.init();
    this.loadDashboard();
  }
  ...
  loadDashboard() {
    ...
    // Atualizar nome genérico no header
    const userNameEl = document.querySelector('.user-info div div:first-child');
    const userEmailEl = document.querySelector('.user-info div div:last-child');
    
    if (userNameEl) {
      userNameEl.textContent = 'Administrador';
    }
    if (userEmailEl) {
      userEmailEl.textContent = 'admin@novamoda.com';
    }
  }
  ...
  init() {
    console.log('🚀 Admin Panel initialized - Acesso livre');
  }
}

// Inicializar apenas se estiver na página admin
if (window.location.pathname.includes('admin')) {
  // Aguarda um pouco para garantir que storage carregou
  setTimeout(() => {
